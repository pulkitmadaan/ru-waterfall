...
#### Reading Files ####
# list.files(ru_path)
sales_raw<- read.csv(paste0(ru_path,"/fsn_fc_pincode_day_sales.csv"),nrows=2000)
...
# Creating combination of dest_pincode, fsn, fc from 
dest_fsn <- sales %>% select(fsn,dest_pincode,brand,bu,category,vertical) %>% distinct %>%
  mutate(dest_pincode=as.factor(dest_pincode))
fc<-data.frame(unique(lzn_mapping_2$fc)) # FCs used for fulfilling to sales dest pincodes
